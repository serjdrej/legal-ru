...
class ThisSkillsOwnActTest(unittest.TestCase):
    ...
    does not prove it reaches legal-ru's own material. This does.
    """

    def test_this_skills_key_act_resolves_and_its_article_extracts(self) -> None:
        try:
            found = ips_lookup.document_text("102108261")
        except IpsError as exc:
            raise unittest.SkipTest(f"pravo.gov.ru IPS unreachable: {exc}") from exc
        body = ips_lookup.extract_article(found["text"], "3")
        self.assertIsNotNone(
            body, "статья 3 не извлеклась из 152-ФЗ «О персональных данных»"
        )
        self.assertIn("персональных данных", body.lower(), body[:200])
        self.assertLess(
            len(body),
            len(found["text"]) // 2,
