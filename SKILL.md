...
- **Не заявлять покрытие, которого нет.** Это ядро на четыре области из
  большего брифа (`docs/BRIEF.md`) — не law-student, не litigation в полном
  объёме, не IP (см. `patent-ru`), не полный ТК РФ (только точки
  соприкосновения с четырьмя ветками выше).
